...
from app.db.models.scheme import Data, InputData, Task, TaskDataDependency
...
class DataRepository(BaseRepository[Data]):
    ...
    async def get_data_job_id_by_parent_task(self, data_id: int):
        statement = (
            select(Task.job_id)
    ...
    async def get_data_job_id_by_dependent_tasks(self, data_id: int):
        statement = (
            select(Task.job_id)
